...
def setup_ci_cd_integration():
    """Setup CI/CD integration"""
    try:
        print("Setting up CI/CD integration...")
        send_telegram("🔄 <b>THIẾT LẬP CI/CD:</b> Đang cấu hình tích hợp CI/CD...")
        
        # Run CI/CD integration
        subprocess.run(["python", "app/scripts/multi_cicd_integration.py"], 
                      capture_output=True, text=True, timeout=300)
        
        send_telegram("✅ <b>CI/CD ĐÃ SẴN SÀNG:</b> Đã cấu hình tích hợp CI/CD")
        return True
    except Exception as e:
        print(f"Error setting up CI/CD integration: {e}")
        send_telegram(f"❌ <b>LỖI CI/CD:</b> {e}")
        return False
...
def run_security_analysis():
    ...
    try:
        ...
        send_telegram("🔍 <b>PHÂN TÍCH BẢO MẬT:</b> Đang chạy các công cụ phân tích bảo mật...")
        
        # Run static analyzer
        subprocess.run(["python", "app/scripts/advanced_static_analyzer.py"], 
                      capture_output=True, text=True, timeout=600)
        
        send_telegram("✅ <b>PHÂN TÍCH BẢO MẬT HOÀN TẤT:</b> Đã hoàn tất phân tích bảo mật")
        return True
    except Exception as e:
        print(f"Error running security analysis: {e}")
        send_telegram(f"❌ <b>LỖI PHÂN TÍCH:</b> {e}")
        return False
